...
### Changed
- GitHub Actions setup/cache and browser-evidence upload now use immutable Node 24-based `actions/setup-node@v7.0.0` and `actions/upload-artifact@v7.0.1` releases instead of the deprecated Node action runtimes that emitted `punycode` and `url.parse()` warnings.
