...
This ledger records PolicyWeave's current commercialization gap from the active PR, product requirements, ADRs, implementation, review findings, and exact-head GitHub evidence. It separates implemented behavior from planned backend, legal-research, security, and release work.
...
| Review workspace | Live preview, warning navigation, responsive review pane | Implemented on active PR head | No screenshot/browser matrix evidence yet | Add Playwright visual/interaction tests at desktop, tablet and mobile breakpoints |
...
| CI | lint/test/build plus security/SAST workflows | Partial | Exact-head runs for the repaired head are queued | Require exact-head success before merge |
...
1. Merge PR #1 only after its current exact-head CI/security/SAST checks succeed.
